...
def asgi_app():
    ...
    async def elevenlabs_webhook(request: Request):
        try:
            ...
            # ElevenLabs Server Webhook payload structure usually puts messages or context in "conversation_history"
            # or we can extract the last user message.
            agent_id = body.get("agent_id", "")
            conversation_id = body.get("conversation_id", "")
            
            # The exact shape depends on the ElevenLabs tool call, but the simplest integration
            # passes the prompt via dynamic variables or direct query.
            # Assuming standard OpenAI style messages are sent in webhook form too:
            messages = body.get("messages", [])
            
            system_prompt = "You are a helpful medical assistant."
            user_query = ""
            
            # Sometimes webhooks send dynamic_variables
    ...
    async def chat_completions(request: Request):
        try:
            ...
            async def event_stream():
                ...
                # remote_gen.aio() cannot be used inside an async generator — Modal's
                # synchronicity raises "aclose(): asynchronous generator is already running".
                # Instead, await the full response and stream word-by-word.
                response_text = await llm.generate_response.remote.aio(system_prompt, user_query, temperature=0.1, max_new_tokens=256)
