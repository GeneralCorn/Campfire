...
MODAL_VISUALIZER_URL  = os.environ.get("MODAL_VISUALIZER_URL", "")

async def call_medication_visualizer(medications: list, patient_name: str = "") -> Optional[dict]:
    """Call the Modal medication visualizer and return image + schedule JSON."""
    if not MODAL_VISUALIZER_URL:
        return None
    try:
        async with httpx.AsyncClient(timeout=45.0) as client:
            response = await client.post(
                MODAL_VISUALIZER_URL.rstrip("/") + "/visualize",
                json={"medications": medications, "patient_name": patient_name},
            )
            response.raise_for_status()
            return response.json()
    except Exception as e:
        print(f"[Visualizer] Error calling Modal visualizer: {e}")
        return None
...
def init_faiss():
    global _INDEX, _corpus_vecs
    if _INDEX is not None:
        return
    embed = get_embed()
    _corpus_vecs = embed.encode(
        _build_corpus(DISCHARGE), normalize_embeddings=True
    ).astype("float32")
    _INDEX = faiss.IndexFlatIP(_corpus_vecs.shape[1])
...
class GraphState(TypedDict):
    ...
    audio_text: str
    ui_trigger: str
    # Visual schedule generated by the medication agent tool call
...
async def medication_agent(state: GraphState) -> dict:
    ...
    # ── Tool call: run LLM + visual generator in parallel ─────────────────────
    patient_name = DISCHARGE.get("patient_profile", {}).get("name", "")
    llm_response, visual_data = await asyncio.gather(
        call_medical_llm("medication", state["query"], system_prompt),
        call_medication_visualizer(DISCHARGE.get("medications", []), patient_name),
    )
...
def join_node(state: GraphState) -> dict:
    ...
    if state.get("medication_result"):
        ...
        visual_data = state["medication_result"].get("visual_data")
...
class OrchestrateResponse(BaseModel):
    ...
    route: str
    off_topic: bool
    visual_data: dict | None = None   # medication schedule PNG + structured slots
